...
2. `action` 값을 아래 실제 웹앱 URL로 설정합니다.
   - `https://script.google.com/macros/s/AKfycbxb369-hV_my_m3MiquRbmaUyZMfhGOLcTggPod8Vp9ANOPLcxA4sYPcIKehpSOep47MA/exec`
...
## 8) Google Apps Script 운영 코드 설정 및 재배포

`google-apps-script/Code.gs`는 아래 상수를 기준으로 동작합니다.

- `SPREADSHEET_ID`: 저장할 구글시트 ID
- `SHEET_NAME`: 저장 탭 이름 (`지원서`)
- `SUCCESS_URL`: 접수 후 이동할 완료 화면
- `FORM_URL`: `doGet` 접근 시 이동할 지원 폼 URL
- `ALERT_EMAIL`: 접수 알림 수신 이메일 (`sckim223@gmail.com`)

동작 순서:
1. `doPost(e)`에서 시트에 저장
2. `[MRA 지원서 접수] 성명 / 휴대전화` 제목으로 메일 발송
3. `SUCCESS_URL`로 즉시 리디렉션

### 반영 순서
1. Apps Script 편집기에 `google-apps-script/Code.gs` 내용을 붙여넣습니다.
2. `SPREADSHEET_ID`를 실제 운영 시트 ID로 교체합니다.
