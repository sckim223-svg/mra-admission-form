...
- `success.html` 이동은 Google Apps Script의 `SUCCESS_URL` 설정으로 제어합니다.
...
저장소의 `google-apps-script/Code.gs` 예시 코드를 Apps Script 프로젝트에 반영하면,
구글시트 저장 직후 아래 주소로 알림 메일이 발송됩니다.

- 수신 이메일: `sckim223@gmail.com`
- 메일 제목: `[MRA 지원서 접수] 새 지원서가 접수되었습니다`

### 반영 순서
1. Apps Script 편집기에 `google-apps-script/Code.gs` 내용을 붙여넣습니다.
2. `SPREADSHEET_ID`, `SHEET_NAME`, `SUCCESS_URL`을 실제 운영값으로 수정합니다.
