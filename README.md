...
2. `action` 값을 아래 실제 웹앱 URL로 설정합니다.
   - `https://script.google.com/macros/s/AKfycbw5VQtUiv2mLZA4hk_S2NqkuPEOa4G6lP7GeI5IedqzI6Xojufmd_KGcS3fp35cd5Zrdw/exec`
...
## 8) Google Apps Script 운영 코드 설정 및 재배포 (중요)

현재 `google-apps-script/Code.gs`는 `SpreadsheetApp.openById(...)`를 쓰지 않고,
**바운드 스크립트 기준 `SpreadsheetApp.getActiveSpreadsheet()`** 로 시트를 찾습니다.

즉, Apps Script 프로젝트는 반드시 실제 운영 시트(`MRA 지원서`)에 바운드된 상태여야 합니다.

사용 상수:
- `SHEET_NAME`: 저장 탭 이름 (`지원서`)
- `FORM_URL`: `doGet` 접근 시 이동할 지원 폼 URL
- `ALERT_EMAIL`: 접수 알림 수신 이메일 (`sckim223@gmail.com`)

동작 순서:
1. `doPost(e)`에서 `지원서` 시트에 저장
2. `[MRA 지원서 접수] 성명 / 휴대전화` 제목으로 메일 발송
3. 완료 HTML 안내 문구 출력

### 반영 순서
1. 구글시트 **MRA 지원서**를 연 뒤, 메뉴 **확장 프로그램 → Apps Script**로 들어갑니다. (중요: 바운드 스크립트)
2. `google-apps-script/Code.gs` 전체를 붙여넣고 저장합니다.
3. **배포 → 배포 관리 → 수정(Edit)** 에서 새 버전으로 다시 배포합니다.
4. 최초 1회는 아래 권한 승인이 필요합니다.
   - Google Sheets 접근 권한
   - MailApp 메일 발송 권한

### 오류 해결 체크리스트 (Illegal spreadsheet id or key)
- 해당 오류가 보이면, 운영 중인 웹앱이 아직 **예전 openById 코드 버전**일 가능성이 큽니다.
- 위 반영 순서대로 코드 교체 후 **웹앱 재배포**를 반드시 다시 진행하세요.
- `apply.html`의 form action URL이 현재 배포된 웹앱 `/exec` URL과 동일한지 확인하세요.
